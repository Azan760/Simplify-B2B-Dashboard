    ...
    return (
        <>
            ...
            <div style={{ borderRadius: '10px', boxShadow: '0 0 10px #172b4d1a' }}
                ...
                <table style={{ borderRadius: '10px', boxShadow: '0 0 10px #172b4d1a' }}
                        ...
                        <tr >
                            {tables[0]?.heading.map((head, index) => {
                                return (
                                    ...
                                    < th key={index} className={`p-2.5   ${index === 4 ? 'text-center' : 'text-left'}    text-white text-xs font-semibold `}>
                                         {head}
                                  </th>
                                )
                            })
                    ...
                    <tbody className='rounded border border-searchIcon box-border shadow-sideShadow'>
                        ...
                        <tr key={index} className='border border-b-2  border-b-searchIcon '>
                            ...
                            <td className='p-2.5 text-textColor2 ' style={{ fontSize: '12.5px' }}>{ FormattedDate(row?.invoiceDetails?.date) }  </td>
                            <td className='p-2.5 text-textColor2 ' style={{ fontSize: '12.5px' }}>
                                <Link className='text-downIcon  decoration-0 
                              font-medium '>
                                {row?.clientDetails?.clientName}
                                </Link>
                            </td>
                            <td className='p-2.5 text-textColor2 ' style={{ fontSize: '12.5px' }}> {row?.totalAmount?.toLocaleString()}
 </td>
                            <td className='p-2.5 text-textColor2 ' >
                                <span className={` text-xs text-white rounded md:py-1 md:px-0 block text-center py-1 px-1.5 bg-textColor2 `}>
                                  Approved
                                </span>
                            </td>
                        </tr>
