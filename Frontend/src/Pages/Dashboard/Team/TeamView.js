...
const TeamView = () => {
    ...
    const { id } = useParams();
    const { getFetch, updateFetch, deleteFetch } = useFetch(`http://localhost:8000/team/view/${id}`); // ADD deleteFetch
    ...
    const deleteMutation = useMutation(
        ...
        {
            onSuccess: () => {
                // Redirect to team list after successful deletion
                navigate('/team/list');
                        ...
                        <div className='flex justify-between button'>
                            ...
                            <div className='flex button'>
                                ...
                                <Button
                                    ...
                                    icon={deleteIcon} 
                                    className={`flex border text-white bg-reds border-searchIcon ${deleteMutation.isLoading ? 'opacity-50' : ''}`}
                                />
                                <Button
                                    type="submit" icon={refresh} label="Update" onSubmit={reset} disabled={!isDirty || isSubmitting}
                                    className={` ${(!isDirty || isSubmitting) && 'opacity-50'} flex inner-btn  bg-textColor text-white  ml-3`}
