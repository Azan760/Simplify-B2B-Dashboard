...
export const GetPurchaseOrder = asyncHandler(async (req, res) => {
    try {
        const purchaseOrders = await PurchaseOrder.find({}).select("-__v");
        if (!purchaseOrders) {
            return res.status(404).json(new ApiError(404, "No purchase orders found"));
        }
        return res.status(200).json(new ApiResponse(200, purchaseOrders, "Purchase orders fetched successfully"));
    } catch (error) {
